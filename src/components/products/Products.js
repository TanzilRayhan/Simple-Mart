...
const Products = async () => {
    ...
    // All Products data fetching
    const data = await GetAllData();
    console.log(data);

    return (
        <>
            <h1 className="text-center bg-slate-300 rounded-lg p-4 font-bold text-5xl my-20">All Products</h1>
                            ...
                            <Link href={`/products/${item.id}`}>
                                <div className="card card-compact h-96 bg-base-100 shadow-xl">
                                    ...
                                    <div className="card-body h-2/4">
                                        <h2 className="text-base text-center font-bold">{item.title}</h2>
